...
TIMER2_RATE   EQU 5000     ; 1000Hz, for a timer tick of 1ms
...
Custom_Characters:
	WriteData(#40h) ; Custom characters are stored starting at address 40h
...
main:
	...
	mov Minute, #0x59
	mov Hour, #0x12
	clr amORpm	; 0 for am, 1 for pm
	clr pause_flag	; 0 for not paused, 1 for paused 
...
loop_b:
	...
	cjne a, #0x60, AAA
	anl a, #0x0	
	mov BCD_counter, a	;reset BCD_counter
	mov a, Minute
	add a, #0x1
	da a
	mov Minute, a
	...
	cjne a, #0x13, AAA
	anl a, #0x1
	mov Hour, a		;reset Hour
	
	jnb amORpm, BBB
	clr amOrpm
	sjmp AAA
BBB:
	setb amORpm	; change to am/pm
...
QQQ:
	...
	ljmp DDD
WWW:
	jnb bottom1, $	; wait for the buttom to be released
	clr TR2	;disable timer
timer_config:
	setb pause_flag
	;clr amORpm
